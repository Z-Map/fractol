...
/*   Updated: 2016/12/15 00:40:19 by qloubier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mglwin		*mglw_mkwin(mglwm mode, mglwf flags)
{
	...
	wdata = (mglwindata){ .user = 1, .state = 0, .window = NULL,
		...
		.layer2D = NULL, .optime = 0.0,
		.kcb = {NULL, NULL, NULL},
		.kcb_args = {NULL, NULL, NULL} };
...
mglwin		*mglw_openwin(mglwin *win, int x, int y, const char *title)
{
	...
	if ((win) && !(win->data->state & 1) && (MGLWtcb[m].opener))
